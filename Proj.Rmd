...
start=Sys.time()
dff$TotalTime = finish_times(arrivals = as.numeric(dff$ArrivalTime)-1501563500, service = dff$ServerTime*3600, due = as.numeric(dff$DueTime)-1501563500, servers = 175)$system_time
Sys.time()-start
...
mean(dff$OnTime[seq(1,100000)])
mean(dff$BeforeLastHour[seq(1,100000)])
mean(dff$BeforeLast4Hour[seq(1,100000)])
...
start=Sys.time()
algorithm = finish_times(arrivals = as.numeric(dffd$ArrivalTime)-1501563500, service = dffd$ServerTime*3600, due = as.numeric(dffd$DueTime)-1501563500, servers = 150)
Sys.time()-start
dffd$TotalTime = algorithm$system_time
dffd$FinishTime = dffd$ArrivalTime + dffd$TotalTime
dffd$StartTime = dffd$FinishTime - 3600*dffd$ServerTime
dffd$SevrerNum = algorithm$server_num
...
dffd$Idle_Num = algorithm$Idle_Num

mean(dffd$OnTime[seq(1,56450)])
mean(dffd$BeforeLastHour[seq(1,60000)])
mean(dffd$BeforeLast4Hour[seq(1,60000)])

max(which(dffd$Idle_Num != 0))
...
for (j in 1:length(arr2)){
  if (j%%1000 == 0){
    ...
    if (queue_times[queue] < arrivals[1]){
      ...
      output[minC(which(arr2==arrivals[i])[arr[which(arr2==arrivals[i])]==arrivals[i]])] = queue_times[queue]
      output[minC(which(arr2==arrivals[i])[arr[which(arr2==arrivals[i])]==arrivals[i]])+n] = queue + 1
      arr2[minC(which(arr2==arrivals[i])[arr[which(arr2==arrivals[i])]==arrivals[i]])]=queue_times[queue]
    } else{
