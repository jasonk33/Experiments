...
dff = df2[1:60000,]
dff$TotalTime = finish_times(arrivals = as.numeric(dff$ArrivalTime)-1501563500, service = dff$ServerTime*3600, due = as.numeric(dff$DueTime)-1501563500, servers = 150)$system_time
...
for (j in 1:length(arr2)){
  if (j%%100 == 0){
    ...
    if (queue_times[queue] < arrivals[1]){
      i=which.min(due2[arrivals[1]>=arrivals])
...
sum(dff$StartTime[1:58000] < dff$ArrivalTime[1:58000])
