...
start=Sys.time()
algorithm2 = finish_times(arrivals = as.numeric(dff$ArrivalTime)-1501563500, service = dff$ServerTime*3600, due = as.numeric(dff$DueTime)-1501563500, servers = 175)
Sys.time()-start
...
sum(dff$ServerTime>dff$TotalTime)

dffd = df2

start=Sys.time()
algorithm = finish_times(arrivals = as.numeric(dffd$ArrivalTime)-1501563500, service = dffd$ServerTime*3600, due = as.numeric(dffd$DueTime)-1501563500, servers = 175)
Sys.time()-start
...
mean(dffd$BeforeLastHour[seq(1,393000)])
mean(dffd$BeforeLast4Hour[seq(1,393000)])
...
for (j in 1:length(arr2)){
  if (j%%1000 == 0){
    ...
    if (queue_times[queue] < minC(arrivals)){
      i=which(arrivals==minC(arrivals))[which_minC(due[which(arrivals==minC(arrivals))])]
      idle[i] = sum(queue_times < arrivals[i])
      ServerNum[i] = queue
      queue_times[queue] = arrivals[i] + service[i]
      output[i] = queue_times[queue]
      output[i+n] = queue + 1
      arr2[i]=queue_times[queue]
    } else{
       i = which(arrivals<=queue_times[queue])[which_minC(due[which(arrivals<=queue_times[queue])])]
       idle[i] = sum(queue_times < arrivals[i])
        ServerNum[i] = queue
